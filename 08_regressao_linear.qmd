...
```{r}
nomes_col_pnud_muni_2010 |> 
  dplyr::filter(sigla == "mort5" | sigla == "t_agua") |> 
  knitr::kable()
```
...
O gráfico a seguir mostra a relação entre a taxa de mortalidade infantil e o percentual de acesso à água encanada, mantendo a renda per capita constante:
...
O gráfico a seguir mostra a relação entre a taxa de mortalidade infantil e a renda per capita, mantendo o percentual de acesso à água encanada constante:
...
### Modelando a variável `rdpc` em logaritmo
...
```{r}
modelo_linear_multiplo_log <- lm(mort5 ~ t_agua + rdpc_log , data = pnud_muni_2010_com_log)
```
...
Vamos visualizar o ajuste do modelo (similar ao que fizemos anteriormente): 
```{r}

pnud_muni_2010_ajustado_agua <- pnud_muni_2010_com_log |> 
  dplyr::mutate(
    rdpc_log = mean(rdpc_log),
  )

pnud_muni_2010_ajustado_rdpc <- pnud_muni_2010_com_log |>
  dplyr::mutate(
    t_agua = mean(t_agua)
  )
```
...
pnud_muni_2010_ajustado_multiplo_log <- pnud_muni_2010_com_log |>
  dplyr::mutate(
    valores_ajustados_agua = predict(modelo_linear_multiplo_log, newdata = pnud_muni_2010_ajustado_agua),
    valores_ajustados_rdpc = predict(modelo_linear_multiplo_log, newdata = pnud_muni_2010_ajustado_rdpc),
...
Os gráficos indicam que o resultado do modelo ajustado com a variável `rdpc` transformada em logaritmo é melhor do que o modelo anterior. Vamos seguir com a análise do modelo ajustado.
