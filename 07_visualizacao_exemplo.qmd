...
Agora aproveitaremos o exemplo anterior para demonstrar o funcionamnto do {ipeaplot}. Porém, primeiro precisamos fazer algumas pequenas modificações no código orginal do gráfico para atender à estrutura de funcionamento do pacote:

```{r}
#| label: exemplo-ipeaplor-base

dados_preparados_ipeaplot <- dados_preparados |> 
  mutate( 
...
grafico_ipeaplot <- dados_preparados_ipeaplot |>
  ggplot(aes(fill = categoria_fct)) + # <1>
  aes(y = perc, x = uf_fct ) + # <2>
  geom_col() +
...
1.  Incluimos o `aes()` com `fill` dentro da função `ggplot()`
2.  Trocamos a posição dos dados nos eixos
3.  Alteramos os labels
4.  usamos a função `coord_flip()`

Notem que o resultado final ainda é examente igual ao exemplo anterior. Agora vamos começar a modificar a figura, usando o {ipeaplot} para ajustar ao "padrão do Ipea". Primeiro usamos a função `theme_ipea()` no lugar de `theme_minimal()`.
...
1.  Usando a pleta de cores azul ("TD") 
...
Por trás das cortinas, as duas funções acima são semelhantes à `ggsave()`, usada anteriormente nesta aula. Porém, os atributos passados garantem que os gráficos serão salvos em formatos "editáveis" (`.eps` ou `.pdf`) e com proporção e resolução recomendadas pelo Editorial do Ipea.