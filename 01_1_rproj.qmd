...
Agora que já sabemos como criar um projeto no RStudio, vamos criar um projeto para o curso. Esse projeto será usado em todas as aulas.

1. Crie um projeto no RStudio, seguindo os passos acima. Salve o projeto em uma pasta onde seja fácil de encontrar o projeto depois.
...
3. Vamos criar algumas pastas para manter o conteúdo organizado. Essas pastas devem ser criadas dentro do projeto que acabamos de criar. Você pode criar as pastas diretamente no seu computador, ou pode criar as pastas usando a função `dir.create("nome_da_pasta")`.
    - `dados/`: bases de dados brutas que usaremos no curso.
    - `dados-output/`: bases de dados que criaremos no curso (ex: bases de dados arrumadas).
