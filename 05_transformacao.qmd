...
Antes de filtrar, podemos consultar quais são os valores distintos possíveis para garantir que selecionamos o correto ao filtrar. As variáveis disponíveis na base de dados, e suas unidades de medida são:
...
A função `pivot_wider()` é útil quando queremos reorganizar uma tabela para que cada linha represente uma unidade de interesse (neste caso, uma combinação de UF e trimestre) e as variáveis categóricas se tornem colunas. Por exemplo, no formato atual (dados longos), temos uma linha para cada combinação de UF, trimestre e condição de ocupação. Ao usarmos `pivot_wider()`, vamos transformar isso para que cada linha represente uma UF por trimestre, e as diferentes condições de ocupação (empregado, desocupado, etc.) se tornem colunas. Isso facilita cálculos agregados, como a taxa de desocupação.
...
Nos exemplos anteriores, usamos uma função por vez, para explorar o que cada uma faz. Porém, conforme ficamos mais confortáveis com o uso das funções, podemos encadear várias funções em uma única linha de código, usando o operador pipe (`%>%` ou `|>`).
...
O `left_join()` é usado aqui para combinar os dados que já estamos trabalhando com os dados das delimitações das UFs. Cada linha do nosso dataset de desocupação será associada à respectiva geometria da UF, permitindo visualizarmos as taxas de desocupação em um mapa. Esse tipo de operação é comum quando queremos realizar análises espaciais, como identificar padrões regionais.
