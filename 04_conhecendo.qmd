...
::: callout
**Observação:** Se você estiver com o `{tidyverse}` carregado, a função `view()` (com o v minúsculo) também está disponível. Ela é uma versão mais moderna da função `View()`, e tem algumas funcionalidades a mais. Para os fins desse tutorial, podemos usar qualquer uma das duas.
:::
...
O resultado é o mesmo, mas dessa forma utilizamos a tradicional string para acessar a coluna.
