...
::: callout
Conceitos prévios importantes para essa aula:

- Pacotes, funções e objetos

- Caminhos de arquivos
:::

O objetivo da etapa de importação é carregar a base de dados para o *Environment* do R.

Essa etapa pode ser bem rápida (ou não!). Depende da base de dados que precisamos carregar.

Nesta etapa, vamos aprender:

-   Como importar dados de arquivos locais (arquivos que estão salvos no nosso computador);

-   Como salvar dados localmente (salvar objetos do R em arquivos que ficarão salvos no nosso computador);
...
Para salvar dados localmente, você pode usar funções com o prefixo `write_` de pacotes como `{readr}` e `{writexl}`. Essas funções solicitam dois argumentos principais: - Qual é o objeto que você queremos salvar? Esse objeto deve estar carregado no *Environment*. - Onde queremos salvar o arquivo? O caminho completo do arquivo deve ser fornecido entre aspas, contendo o diretório onde ele será salvo (em qual pasta do computador) e o nome do arquivo (com a extensão, por exemplo, `.csv`).
...
A função `write_rds()` do pacote `{readr}` permite salvar um objeto do R em um arquivo binário do R.
...
É recomendável que você salve uma cópia local dos dados brutos após o download, e use-os para trabalhar localmente. Isso evita que você precise baixar os dados toda vez que for trabalhar com eles. 
:::
...
Podemos fazer isso com a função `strsplit()`:

```{r}
periodos_disponiveis <- strsplit(info_4092$period, ", ")[[1]]
