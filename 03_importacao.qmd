...
Para os exemplos de importação de dados, vamos utilizar alguns arquivos que estão disponíveis no repositório do curso.

Para fazer download dos arquivos, verifique se está no projeto criado para o curso, e se sim, copie o código e abaixo, cole e execute-o no console:
...
#| eval: false
library(writexl)
write_xlsx(dados, "dados_output/sidrar_4092_bruto.csv")
```
...
dados_brutos_4092 <- sidrar::get_sidra(
  x = 4092,                      # <1>
  period = periodos_disponiveis, # <2>
  geo = "State"                  # <3>
)

```
1. Qual é o código da tabela que vamos usar?
2. Períodos: trimestres, vamos usar o vetor criado anteriormente
